// src/screens/BackupSendersScreen.js - CLEANED VERSION (No device contacts)
...
const BackupSendersScreen = ({navigation}) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Only use recent chats (no device contacts)
      const items = recents.map(chat => ({...chat, type: 'recent'}));
      ...
      let filtered = items;
      if (selectedApp !== 'all') {
        filtered = items.filter(item => item.app === selectedApp);
      }
  ...
  const renderItem = ({item}) => {
    const app = item.app;
    const identifier = item.contact_identifier;
    const name = item.name || 'Unknown Sender';
    const key = `${app}_${identifier}`;
        ...
        <View style={styles.chatRow}>
          ...
          <View style={styles.chatInfo}>
            ...
            <Text variant="bodySmall" numberOfLines={1}>
              Last:{' '}
              {new Date(Number(item.last_timestamp || 0)).toLocaleDateString()}
            </Text>
          </View>
    ...
    <View
      ...
      {allItems.length === 0 ? (
        <View style={styles.noItemsContainer}>
          <Text variant="bodyMedium">
            {selectedApp === 'all'
              ? 'No messages yet. Send or receive messages in enabled apps to see them here.'
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            `${item.type}_${item.app}_${item.contact_identifier || index}`
          }
